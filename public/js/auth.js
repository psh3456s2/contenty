  ...
  if (mode === 'login') {
    content.innerHTML = `
      ...
      <div class="auth-form">
        ...
        <p class="auth-switch" style="margin-top:0.5rem;"><button onclick="renderAuthContent('reset')">비밀번호를 잊으셨나요?</button></p>
    ...
    content.innerHTML = `
      ...
      <p class="auth-sub">가입한 이메일로 재설정 링크를 보내드려요 📩</p>
      <div id="authMsg"></div>
      <div class="auth-form">
        <input class="auth-input" type="email" id="authEmail" placeholder="이메일" autocomplete="email" />
        <button class="btn-auth-primary" id="authSubmitBtn">재설정 링크 보내기</button>
        <p class="auth-switch" style="margin-top:0.75rem;">로그인으로 돌아가기 <button onclick="renderAuthContent('login')">로그인</button></p>
      </div>
    `;
  } else {
    content.innerHTML = `
...
function setAuthLoading(loading, mode) {
  ...
  if (!btn) return;
  const labels = { login: '로그인', signup: '회원가입', reset: '재설정 링크 보내기' };
  if (loading) {
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner"></span>처리 중...';
  } else {
    btn.disabled = false;
    btn.textContent = labels[mode] || '확인';
  }
}
...
async function handlePasswordReset() {
  const email = document.getElementById('authEmail')?.value?.trim();
  if (!email) { setAuthMsg('가입하신 이메일을 입력해주세요.'); return; }
  ...
  if (error) {
    ...
    setAuthMsg('재설정 링크를 이메일로 보냈어요! 메일함을 확인해주세요. 📩', 'success');
  }
}
